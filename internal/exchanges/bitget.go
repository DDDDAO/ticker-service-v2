...
func (h *BitgetHandler) Subscribe(symbol string) {
	...
	for _, s := range h.config.Symbols {
		if s == symbol {
			h.mu.Unlock()
			logger.WithSymbol("bitget", symbol).Info("Already subscribed to symbol")
			return
		}
	}
	...
	logger.WithSymbol("bitget", symbol).Info("Adding symbol - reconnecting WebSocket")
...
func (h *BitgetHandler) Unsubscribe(symbol string) {
	...
	if !found {
		h.mu.Unlock()
		logger.WithSymbol("bitget", symbol).Warn("Symbol not in subscription list")
		return
	}
	...
	logger.WithSymbol("bitget", symbol).Info("Removing symbol - reconnecting WebSocket")
...
func (h *BitgetHandler) reconnect() {
	...
	if err := h.Connect(); err != nil {
		logger.WithExchange("bitget").Errorf("Failed to reconnect: %v", err)
	}
}
